const logoPaths = {
  CSE: require("../assets/logos/CSE.png"),
  EE: require("../assets/logos/EE.png"),
  ECE_META: require("../assets/logos/ECE_META.png"),
  MM: require("../assets/logos/ECE_META.png"),
  //ECE_META: require("../assets/logos/ECE+META.png"),
  // MM: require("../assets/logos/ECE+META.png"),
  CIVIL: require("../assets/logos/CIVIL.png"),
  MECH: require("../assets/logos/MECH.png"),
  MSc_ITEP: require("../assets/logos/MSc.png"),
  MTech: require("../assets/logos/MTech.png"),
  PHD: require("../assets/logos/PhD.png"),
};

export default logoPaths;
